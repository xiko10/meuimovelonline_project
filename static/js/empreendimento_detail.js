...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ========================
// FUNCIONALIDADE DOS CARDS EXPLORE
// ========================
...
if (exploreCards.length > 0) {
  exploreCards.forEach(card => {
    card.addEventListener('click', () => {
      ...
      const correspondingButton = document.querySelector(`.btn-midia[data-modal-type="${modalType}"]`);
  ...
  const btnValoresReservar = document.getElementById('btn-valores-reservar');
  const modalReserva = document.getElementById('modalReserva');
  const modalObrigado = document.getElementById('modalObrigado');
  const modalOverlay = document.getElementById('modalOverlay');
  const modalClose = document.getElementById('modalClose');
  const formReserva = document.getElementById('formReserva');
  const btnCorretor = document.getElementById('btn-corretor');
  const modalQr = document.getElementById('modalQrCorretor');

  if (btnValoresReservar && modalReserva && modalObrigado && formReserva) {
    btnValoresReservar.addEventListener('click', () => {
    ...
    formReserva.addEventListener('submit', async (e) => {
      ...
      const nome = formReserva.nome.value;
      const whatsapp = formReserva.whatsapp.value;

      try {
        await fetch("https://formsubmit.co/ajax/leads@teste.com.br", {
          method: "POST",
          headers: {
  ...
  // QR Modal interação
  if (btnCorretor && modalQr) {
    btnCorretor.addEventListener('mouseenter', (e) => {
      const rect = btnCorretor.getBoundingClientRect();
      modalQr.style.top = rect.bottom + window.scrollY + 8 + 'px';
      modalQr.style.left = rect.left + 'px';
      modalQr.classList.remove('hidden');
    });

    btnCorretor.addEventListener('mouseleave', () => {
      setTimeout(() => {
        if (!modalQr.matches(':hover')) {
          modalQr.classList.add('hidden');
        }
      }, 200);
    });

    modalQr.addEventListener('mouseleave', () => {
      modalQr.classList.add('hidden');
    });

    document.addEventListener('click', (e) => {
      if (!btnCorretor.contains(e.target) && !modalQr.contains(e.target)) {
        modalQr.classList.add('hidden');
      }
    });
  }